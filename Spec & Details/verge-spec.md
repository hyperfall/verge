...
| **M2** | L1 + memory + L3 — perception → retrieval → reasoner | next | retrieval measurably improves reasoning vs a no-retrieval ablation; **predictive (lookahead) recall beats reactive top-k at a smaller working set**, and the recall gate trains offline without loading the base (decoupled) |
